...
export function botSystemPrompt(
  ...
  const have = [
    ...
    agents &&
      "Questions — `ask_user(question, choices, multiple, detail)` is the ONLY way to ask the human anything. Whenever you lack a decision or data you cannot obtain yourself, call it: one question per call, with 2-5 ready answers as `choices`. `question` is the card's title, so keep it to one short line and put any background in `detail` (optional, shown small underneath). Set `multiple: true` when several of the choices may be picked at once — the card then shows checkboxes and a Confirm button, and the answer comes back as the chosen labels separated by commas; leave it out for a pick-one question. NEVER end a message with a question mark (`?`) in plain text — every question must go through `ask_user` and the human answers via the in-chat card. Do not ask about something a tool can check.",
    agents &&
