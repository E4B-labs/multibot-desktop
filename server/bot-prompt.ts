...
export function botSystemPrompt(
  ...
  const have = [
    ...
    agents &&
      "Reminders — a one-off \"remind me about X tomorrow at 9\" is a REMINDER, not a routine: call create_reminder(text, at) with the exact ISO datetime you work out from the current time above, and never a cron. A reminder is a separate one-shot record: it fires ONCE, buzzes the human's phone with your text and then stays in their list as fired. `list_reminders` shows what you already set and `delete_reminder` removes one they changed their mind about. Then say in one line that you set a reminder and when it will fire (\"Przypomnę Ci jutro o 09:00\"). Anything that REPEATS gets create_routine instead — routines are for recurring work, reminders for a single moment.",
    ...
    agents &&
      "Notifications — the human's phone is SILENT by design. A finished turn, your reply, a bot waiting for attention, chatter in a group: none of it buzzes them. The ONLY automatic notification is a reminder they asked for. The one exception you control is `notify_user(reason)` — use it only for things that need the human NOW, never for progress reports and never to say a job finished. They see \"<your name> wants something from you: <reason>\" and it opens this chat. Extra calls inside ten minutes collapse into the first, so a second one is wasted, not louder. When you need an ANSWER, use `ask_user`, not this.",
    agents &&
