...
Data lives in `~/.multibot/` (legacy-compatible path: bots, transcripts, per-thread NDJSON event logs,
...
- **Never touch the real `~/.multibot`.** The setup file points `HOME` at a temp dir; keep it
  that way.
...
API keys are write-only: they land in `~/.multibot/config.json` via `PUT /api/config` and the API
